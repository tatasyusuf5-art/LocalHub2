...
replacement = """    fun deleteBackgroundImage(context: Context, bgId: String, encryptedPath: String) {
        viewModelScope.launch(Dispatchers.IO) {
            try {
                ...
                if (file.exists()) {
                    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
                        ...
                        val contentValues = ContentValues().apply {
                            put(MediaStore.Images.Media.DISPLAY_NAME, "Arkaplan_${System.currentTimeMillis()}.jpg")
                            put(MediaStore.Images.Media.MIME_TYPE, "image/jpeg")
                            put(MediaStore.Images.Media.RELATIVE_PATH, "DCIM/Restored")
                        ...
                        val outFile = File(restoredDir, "Arkaplan_${System.currentTimeMillis()}.jpg")
...
if target in content:
    ...
    with open("app/src/main/java/com/example/ui/viewmodel/AppViewModel.kt", "w") as f:
        f.write(content)
    print("Replaced deleteBackgroundImage")
else:
    print("Could not find deleteBackgroundImage")
